    ...
    def lookup_team_info(self, field="name_abbrev", lookupfield="team_code", lookupval=None, sport_code="mlb"):
        ...
        if not self.TEAMINFO.get(sport_code):
            try:
                ...
                logging.error("Couldn't connect to MLB server to download team info, returning None: %s",e)
                return -1
            except urllib2.HTTPError, e:
                logging.error("Couldn't download team info, returning None: %s",e)
                return -1
            except Exception as e:
                logging.error("Unknown error downloading team info, returning None: %s",e)
                return -1
        else:
        ...
        logging.error("Couldn't look up %s from %s=%s, returning None...",field, lookupfield, lookupval)
        return None
